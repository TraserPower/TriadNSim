...
namespace TriadCompiler.Parser.Common.ObjectRef
    ...
    class ObjectReference : CommonParser
        ...
        /// <summary>
        /// Проверить число индексов у списка
        /// </summary>
        /// <param name="varType">Заявленный тип объекта</param>
        /// <param name="objRef">Описание фактического вызова</param>
        /// <param name="allowNotIndexedList">Допустимы ли списки без индексов</param>
        public static void CheckIndexCountForList(ICommonType varType, ObjectRefInfo objRef, bool allowNotIndexedList)
        {
            //Если переменная имеет тип список
            if (varType is VarListType)
            {
                VarListType listType = varType as VarListType;
                //Если список используется без индексов
                if (!objRef.HasIndexes && !allowNotIndexedList)
                {
                    Fabric.Instance.ErrReg.Register(Err.Parser.Usage.List.IndexNeeded);
                }
                //Если указаны индексы
                else if (objRef.HasIndexes)
                {
                    //Проверка числа индексов
                    if (objRef.IndexCount != 1)
                    {
                        Fabric.Instance.ErrReg.Register(Err.Parser.Usage.List.TooMuchIndexes);
                    }
                }
            }
        }
