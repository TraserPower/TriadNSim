...
namespace TriadCompiler.Parser.Common.Var
    ...
    internal class Variable : CommonParser
        ...
        public static VarInfo Parse( EndKeyList endKeys, bool allowRange, bool allowNotIndexedArray )
            ...
            // Если список - проверяем индексацию специальной функцией для списка
            if(varInfo.Type is VarListType)
            {
                ObjectReference.CheckIndexCountForList(varInfo.Type, objRef, allowNotIndexedArray);
            }
            // иначе функцией предназначенной для любой переменной
            else
            {
            ...
            if ( (varInfo.Type is VarArrayType || varInfo.Type is VarListType) && objRef.HasIndexes && !objRef.IsRange )
                {
