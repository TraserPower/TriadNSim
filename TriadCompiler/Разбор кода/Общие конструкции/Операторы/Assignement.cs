    ...
    internal class Assignement : CommonParser
        ...
        public static CodeStatementCollection Parse( EndKeyList endKeys, AssignContext context )
            ...
            if ( context == AssignContext.Routine )
                ...
                if ( !( varInfo.Type is VarArrayType || varInfo.Type is VarListType) )
                    {
        ...
        public static bool CheckVarTypes( IExprType varType, IExprType exprType )
            ...
            // Если один из типов - список, а второй нет
            else if (varType is VarListType != exprType is VarListType)
            {
                typesArePermissible = false;
            }
            ...
            //by jum
                ...
                // Если присваиваются списки
                else if (varType is VarListType && exprType is VarListType)
                {
                    //Если типы элементов списков не совпадают
                    if (varType.Code != exprType.Code && varType.Code != TypeCode.UndefinedType)
                    {
                        typesArePermissible = false;
                    }
                }
